...
| [question-clustering-tsp](https://github.com/khshalnuaimi/question-clustering-tsp) | Cluster a corpus of questions by semantic similarity and order each cluster with a from-scratch TSP heuristic — sentence embeddings, k-medoids, nearest-neighbor + 2-opt. |
| [rag-document-qa](https://github.com/khshalnuaimi/rag-document-qa) | Provider-agnostic retrieval-augmented generation for document QA — sentence-aware chunking, a from-scratch NumPy vector store with maximal marginal relevance, cited grounded answers, and a fully offline fallback (extractive backend + hashing embeddings) that runs with no API key. |
| [knowledge-graph-qa](https://github.com/khshalnuaimi/knowledge-graph-qa) | Question answering over a knowledge graph — entity and relation linking, breadth-first shortest-path explanations, type-gated multi-hop reasoning, and a deny-by-default contract that refuses rather than fabricates, with an offline embedding fallback. |
